...

{
    ...
    ID3D11BlendState* pBlendStat = nullptr;
...
HRESULT Direct3D::Initialize(int winW, int winH, HWND hWnd)
{
   ...
   //レンダリング結果を表示する範囲
    ...
    BlendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;     //今描画しようとしてるもの
    ...
    pDevice_->CreateBlendState(&BlendDesc, &pBlendStat);

    float blendFactor[4] = { D3D11_BLEND_ZERO,D3D11_BLEND_ZERO,D3D11_BLEND_ZERO,D3D11_BLEND_ZERO };
    pContext_->OMSetBlendState(pBlendStat, blendFactor, 0xffffffff);
...
HRESULT Direct3D::InitTonnOutLine()
{
    using namespace Direct3D;
    HRESULT hr;
    ...
    hr = pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(),
        pCompilePS->GetBufferSize(), NULL, &(shaderBundle[SHADER_TOONOUTLINE].pPixelShader_));
    if (FAILED(hr))
    {
        //エラー処理
        MessageBox(nullptr, "ピクセルシェーダの作成に失敗しました", "エラー", MB_OK);
...
HRESULT Direct3D::InitNormalMap()
{
    ...
    hr = pDevice_->CreatePixelShader(pCompilePS->GetBufferPointer(),
        pCompilePS->GetBufferSize(), NULL, &(shaderBundle[SHADER_NORMALMAP].pPixelShader_));
    if (FAILED(hr))
    {
        //エラー処理
        MessageBox(nullptr, "ピクセルシェーダの作成に失敗しました", "エラー", MB_OK);
